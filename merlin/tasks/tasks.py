...
from merlin.tasks.chord_lock import DistributedLock
...
def safe_add_to_chord(
    task: Task,
    signature_or_chord: Union[Signature, chord],
    lazy: bool = False, 
    lock_timeout: int = 5,
    max_wait: int = 10,
):
    """
    Thread-safe wrapper for add_to_chord that uses distributed locking.
    
    Args:
        task: The Celery task instance
        signature_or_chord: The signature or chord to add
        lazy: Whether to add lazily
        lock_timeout: Timeout for the distributed lock
    ...
    try:
        ...
        with DistributedLock(chord_id=chord_id, timeout=lock_timeout, max_wait=max_wait):
            LOG.debug(f"Adding to chord with lock: {chord_id}")
            task.add_to_chord(signature_or_chord, lazy=lazy)
            LOG.debug(f"Successfully added to chord: {chord_id}")
            
    except Exception as e:
        LOG.error(f"Failed to add to chord safely: {e}")
        # Fallback: add without lock to prevent total failure
